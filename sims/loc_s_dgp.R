## ---------------------------
##
## Script name: loc_s_dgp.R
##
## Purpose of script: generates simulation data for Gaussian location-scale model for effect selection on basis of the NBPSS prior
...
if (sparsity == "sparse"){
  ...
  eta <- data.frame(f1 = f1, f2 = f2, f3 = f3, f4 = f4, eta = f1 + f2 + f3 + f4, etasd = f1 + f2 + f3 + f4)
...
#  normal location-scale (scaling may be slightly different from original sims )
for(i in 1:R){
  scale <- 5/max(eta$etasd[1:n])
  shift <- -mean(eta$etasd[1:n])*scale + log(4)
  
  eta$etasd <- eta$etasd*scale + shift
  
  eps <- rnorm(n = (n), sd = sqrt(exp(eta$etasd)))
